...
import { Plus, Minus, ShoppingCart, Package, Sparkles, ChevronLeft, ChevronRight, Home, TrendingUp, Leaf, X } from 'lucide-react';
...
export function ProductsScreen() {
  const { products, cart, addToCart, updateQuantity, cartTotal, setCurrentScreen, selectedCategory: contextCategory, setSelectedCategory: setContextCategory } = useFastMarket();
  const [selectedCategory, setSelectedCategory] = React.useState<string>(contextCategory || 'Todos');
  const [currentPage, setCurrentPage] = React.useState(0);
  const [selectedProduct, setSelectedProduct] = React.useState<Product | null>(null);
  const [isModalOpen, setIsModalOpen] = React.useState(false);
  const [searchQuery, setSearchQuery] = React.useState('');

  // Sincronizar com a categoria do contexto
  React.useEffect(() => {
    if (contextCategory) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 via-white to-green-50 flex flex-col">
      ...
      <motion.div
        ...
        className="py-3 px-4 md:py-4 md:px-8 bg-white/90 backdrop-blur-sm border-b border-gray-200 shadow-sm sticky top-0 z-40"
      >
        <div className="flex items-center gap-4">
          <Logo size="md" />
          <div className="flex-1">
            <h1 className="font-bold text-gray-800 text-lg md:text-2xl">
              Selecione os Produtos
            </h1>
            <p className="text-gray-600 text-xs md:text-sm">
              Escolha os itens desejados e adicione ao carrinho
            </p>
          </div>
        </div>
      </motion.div>
      ...
      <div className="flex-1 flex overflow-hidden">
        {/* Sidebar - Categorias + Ações + Carrinho */}
        <motion.div
                  ...
                  >
                    <div className="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden hover:shadow-2xl transition-all duration-300 h-full flex flex-col">
                      ...
                      <div className="p-3 md:p-4 flex flex-col flex-1">
                        <h3 className="font-bold text-gray-800 text-sm md:text-base leading-tight mb-2 h-10 md:h-12 line-clamp-2 overflow-hidden">
                          {product.name}
                        </h3>
